...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        self.gradient['W'] = np.matmul(np.transpose(X),
                                       grad)  # (input_D-by-output_D numpy array, the gradient of the mini-batch loss w.r.t. self.params['W'])
        self.gradient['b'] = np.sum(grad,
                                    axis=0)  # (1-by-output_D numpy array, the gradient of the mini-batch loss w.r.t. self.params['b'])
        backward_output = np.matmul(grad, np.transpose(
            self.params['W']))  # (N-by-input_D numpy array, the gradient of the mini-batch loss w.r.t. X)
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        backward_output = np.multiply(grad, self.mask == X)
        return backward_output
